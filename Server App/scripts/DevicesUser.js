	...
	if (this.readyState == 4 && this.status == 200) {
		...
		if(txt != "[]"){
			...
			for (var i = 0; i < devices.length; i++) {
				...
				if(devices[i].user_id == null || devices[i].user_id == user_id){
...
function SendRequest2(){
	...
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
				window.location.href = server_url + "/DevicesUser/" + project_name + "/" + user_id;
		}		
	}
	...
	if (this.readyState == 4 && this.status == 200) {
		...
		if(txt2 != "[]"){
			...
			for (var i = 0; i < devicesTracking.length; i++) {
				...
				if(devicesTracking[i].user_id == user_id){
