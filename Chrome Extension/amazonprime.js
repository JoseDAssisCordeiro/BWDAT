...
var eps_code = "";
var init_time = "";
var streamer = "Amazon Prime";

chrome.storage.sync.get(['amazonprime'], function(project) {
	
	if(project.amazonprime == 'Yes'){
...
function beforeunloadHandler (){
	...
	if(url.includes("detail") && eps_code != "")
...
function ReadEpisode (){
	...
	if(eps_url[eps_url.length-1] !== 'undefined'){
		...
		if(document.getElementsByTagName('video')[document.getElementsByTagName('video').length - 1] != undefined){
			var times = document.getElementsByTagName('video')[document.getElementsByTagName('video').length - 2]

			if(eps_url[eps_url.length-1].includes("ref="))
				eps_info[0] = eps_url[eps_url.length-2];
			else
